...
export class UIManager {
  ...
  init() {
    this._injectStyles();
    this.hud.build();
    this._buildErrorOverlay();
  }
_buildErrorOverlay() {
    // Catches all JS errors and shows them on screen
    ...
    window.onunhandledrejection = (e) => {
      ...
      div.style.cssText = `
        position:fixed; bottom:0; left:0; right:0;
        background:rgba(150,0,150,0.95); color:#fff;
        font-family:monospace; font-size:11px;
        padding:10px; z-index:99999;
        max-height:40vh; overflow-y:auto;
        white-space:pre-wrap; word-break:break-all;
...
const VN_UI_CSS = `
  ...
  .vn-tut-btns .vn-btn-primary, .vn-tut-btns .vn-btn-secondary { margin:0; flex: 1; width: auto; }
`;
